...
impl<S: UnificationStoreMut> UnificationTable<S> {
    ...
    /// read only, no path compression
    #[inline(always)]
    fn get_inlined_get_root_key(&self, vid: S::Key) -> S::Key {
        let redirect = {
            match self.value(vid).parent(vid) {
                None => return vid,
                Some(redirect) => redirect,
        ...
        self.get_uninlined_get_root_key(redirect)
    }
    ...
    #[inline(never)]
    // read only, no path compression
    fn get_uninlined_get_root_key(&self, vid: S::Key) -> S::Key {
        self.get_inlined_get_root_key(vid)
    }
    ...
    V: UnifyValue,
{
    ...
    /// Given a key, returns the root key ()
    pub fn get_find<K1>(&self, id: K1) -> K
    where
        K1: Into<K>,
    {
        self.get_uninlined_get_root_key(id.into())
    }
